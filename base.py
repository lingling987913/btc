...
@st.cache_data
def upload_temp_docs1(files) -> str:
    '''
    将文件上传到临时目录，用于文件对话
    返回临时向量库ID
    '''
...
def btc_st(is_lite: bool = False):
    ...
    with st.sidebar:
        ...
        running_models = ['交易时间','交易频次','稳定币内部转换交易笔数']
        available_models = []
        config_models = []
        ...
        prompt_templates_kb_list = ['default','other']
        ...
        # kb_top_k = st.number_input("匹配知识条数：", 1, 20, 5)
        # ## Bge 模型会超过1
        # score_threshold = st.slider("知识匹配分数阈值：", 0.0, 2.0, float(0.8), 0.01)
    ...
    if st.button("开始上传", disabled=len(files)==0):
        # st.session_state["file_chat_id"] = upload_temp_docs(files)
        for file_ in files:
            upload_temp_docs(file_)
        files = [convert_file(file) for file in files]
        print(f'convert_file is {files}')
...
def upload_temp_docs(uploaded_file):
